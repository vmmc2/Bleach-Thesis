...
Given the fact that, normally, an academic semester spans through 10 to 16 weeks depending on the internal organization of the university, for the sake of simplicity, it will be assumed that the academic semester spans through 14 weeks.
...
\begin{itemize}
    ...
    \item \textbf{Week 9 (Scopes and Environments):}
        \begin{itemize}
            \item Provides an explanation about scopes (a conceptual idea), blocks (its visual representation) and environments (its respective implementation). 
    ...
    \item \textbf{Week 10 (Control Flow Structures):}
        \begin{itemize}
            ...
            \item Proposition of bonus challenges that make the students think about concepts that have not been directly taught but can be learned through different types of references, such as: Addition of support for \texttt{for} and \texttt{while} loops and also for \texttt{else if} and \texttt{else} clauses as a mean to make the \texttt{if} statement more robust.
        \end{itemize}
    ...
    \item \textbf{Week 11 (Functions):}
        \begin{itemize}
            \item Provides explanation about how function declaration statements and function calls work behind the scenes. As well as how one can implement such concepts.
            \item Proposition of an assignment to extend Bleach to support function declaration statements as well as function calls.
            \item Proposition of a bonus challenge where students need to implement anonymous/lambda functions.
        \end{itemize}

    \item \textbf{Week 12 (Resolving):}
        \begin{itemize}
            \item 
        \end{itemize}
    
    \item \textbf{Week 13 (Object-Oriented Features):}
        \begin{itemize}
            \item 
        \end{itemize}

    \item \textbf{Week 14 (Projects Presentation and Final Exam):}
        \begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Fast Execution and Iteration:}
    \item \textbf{Hands-On Experience:}
    \item \textbf{Flexibility for Future Modifications:}
    \item \textbf{Solid Foundation for Future Learning:}
\end{itemize}
